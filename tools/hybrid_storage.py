...
class HybridStorage(object):
    ...
    @classmethod
    def from_folders(cls, event_folder, gt_image_folder, image_file_template="*.png", gt_img_timestamps_file="ts_frame.txt", event_name='event.npy',IsTXY=True):
        ...
        events = event.EventSequence.from_npy_file(filename=os.path.join(event_folder, event_name),
                                                    image_height=gt_images._height, image_width=gt_images._width,IsTXY=IsTXY)
