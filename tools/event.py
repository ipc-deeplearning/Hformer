...
class EventSequence(object):
    ...
    def filter_by_timestamp(self, start_time, duration, make_deep_copy=True):
        ...
        返回按时间戳筛选的事件序列。
        新序列包括 [start_time， start_time+duration） 中的事件。
        """
    ...
    @classmethod
    def from_npy_file(cls, filename, image_height, image_width, start_time=None, end_time=None,IsTXY=True):
        """Reads event sequence from numpy file list."""
        tmp = np.load(filename, allow_pickle=True)

        #把 p是0或者1调整到-1 和 1 之间
        tmp[:, -1] = tmp[:, -1] * 2 - 1
        if IsTXY:
            features = tmp[:, [1, 2, 0, 3]]  # t,x,y,p -> x,y,t,p
        else:
            # t,y,x,p -> x,y,t,p
            features = tmp[:, [2, 1, 0, 3]]  # t,y,x,p -> x,y,t,p
        return EventSequence(features, image_height, image_width, start_time, end_time)
