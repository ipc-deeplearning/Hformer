...
class EventEncoder(nn.Module):
    def __init__(self, netParams, hidden_number=32, theta=[3, 5, 10], tauSr=[1, 2, 4], tauRef=[1, 2, 4],
        ...
        self.conv1 = nn.Conv2d(hidden_number, 32//2, 7, stride=1, padding=3)
        self.conv2 = nn.Conv2d(32//2, 32//2, 7, stride=1, padding=3)

        self.down1 = down(32//2, 64//2, 5, norm=norm)
        self.down2 = down(64//2, 128//2, 3, norm=norm)
        self.down3 = down(128//2, 256//2, 3, norm=norm)
        self.down4 = down(256//2, 512//2, 3, norm=norm)
    ...
    def forward(self, events, n_left, n_right, forward=True):
        ...
        # s1是不需要接外面的 s2-s5需要接到外面
        return [ s2[:bs, ...], s3[:bs, ...], s4[:bs, ...], s5[:bs, ...]], \
               [ s2[bs:, ...], s3[bs:, ...], s4[bs:, ...], s5[bs:, ...],]
