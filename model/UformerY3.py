...
class UformerY(nn.Module):
    def __init__(self, img_size=256, out_chans=1, dd_in=3,
        ...
        self.output_proj2 = OutputProj(in_channel=2*embed_dim, out_channel=out_chans, kernel_size=3, stride=1)
        ...
        # dcoder2
        self.upsample_10 = upsample(embed_dim*16, embed_dim*8)
        self.decoderlayer_10 = BasicUformerLayer(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[5],
        ...
        self.decoderlayer_11 = BasicUformerLayer(dim=embed_dim*8,
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator)
        self.upsample_12 = upsample(embed_dim*8, embed_dim*2)
        self.decoderlayer_12 = BasicUformerLayer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
                            input_resolution=(img_size // 2,
                                                img_size // 2),
                            depth=depths[7],
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator)
        self.upsample_13 = upsample(embed_dim*4, embed_dim)
        self.decoderlayer_13 = BasicUformerLayer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[8],
    ...
    def forward(self, x,mask=None):
        ...
        #Decoder2
        up10 = self.upsample_10(conv4)
        deconv10 = torch.cat([up10, conv3], -1)
        deconv10 = self.decoderlayer_10(deconv10, mask=mask)

        up11 = self.upsample_11(deconv10)
        deconv11 = torch.cat([up11, conv2], -1)
        deconv11 = self.decoderlayer_11(deconv11, mask=mask)

        up12 = self.upsample_12(deconv11)
        deconv12 = torch.cat([up12, conv1], -1)
        deconv12 = self.decoderlayer_12(deconv12, mask=mask)

        up13 = self.upsample_13(deconv12)
        deconv13 = torch.cat([up13, conv0], -1)
        deconv13 = self.decoderlayer_13(deconv13, mask=mask)

        # Output Projection

        y2 = self.output_proj2(deconv13)
...
#     model_restoration = UformerY(img_size=input_size, embed_dim=16,depths=depths,
#                  win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
#
#     # print(model_restoration)
#
#     x = torch.rand((1,3,256,256))
#     x1,x2 = model_restoration(x)
#     print(x1.shape)


#
#     x = torch.rand((1,3,256,256))
#     print(model_restoration(x).shape)
