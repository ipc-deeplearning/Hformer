...
class Event_2_Image_2_Fusion(nn.Module):
    def __init__(self,embed_dim,img_size):
        ...
        self.conv1 = nn.Conv2d(embed_dim*2,embed_dim//2,3,padding=1)

        # Encoder
        self.fusion1 = BasicUformerLayer(dim=embed_dim//2, output_dim=embed_dim//2, input_resolution=(img_size,img_size), depth=1,
                                                num_heads=4, win_size=8)
    ...
    def forward(self, image_event_fea):
        ...
        output=output1.flatten(2).transpose(1, 2).contiguous()  # B H*W C
        output = self.fusion1(output)
        output = output.transpose(1, 2).contiguous().view(B, -1, H, W)
