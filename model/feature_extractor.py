...
class MotionFormerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size=0, shift_size=0, mlp_ratio=4., bidirectional=True, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
...
class MotionFormer(nn.Module):
    def __init__(self, in_chans=3, embed_dims=[32, 64,128,256,512,512], num_heads=[8, 16],
                 mlp_ratios=[4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 6, 2,2,2,1], window_sizes=[11, 11],**kwarg):
        super().__init__()
        self.depths = depths
            ...
            if i == 0:
                ...
                if i < self.conv_stages:
                    ...
                    block = nn.ModuleList([MotionFormerBlock(
                        dim=embed_dims[i], num_heads=num_heads[i-self.conv_stages], window_size=window_sizes[i-self.conv_stages],
