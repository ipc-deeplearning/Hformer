...
vmax = 0.6; % 动态颗粒的最大速度，pixel/帧
vmin = 0.6; % 动态颗粒的最小速度，pixel/帧
v = repmat(vmax, N1, 2);% 动态颗粒的速度，pixel/帧


Svmax = 0.4; % 动态颗粒的最大速度，pixel/帧
Svmin = 0.4; % 动态颗粒的最小速度，pixel/帧
Sv =  repmat(Svmax,  N2,2); % 动态颗粒的速度，pixel/帧



zmin = 3000; % 样品最近距离，单位：um 
zmax = 5000; % 样品最远距离，单位：um 
...
%%  创建文件夹
% 创建指定文件夹路径
folderPath = './differentFlower/Time22';
...
imwrite(imnoise(uint8((HoloS/2 * 256)),"gaussian",0,noise_var), filename2); 
%  强度到灰度：/2*256，单位：nan，增加高斯噪声noise_var
...
for b = 1:1
    ...
    % 单位：pixel, pixel, um, 绝对折射率, 相对相位,光强衰减系数
        ...
        filename1 = [HoloNPath,num2str(t+(b-1)*T,'%03d'),'.mat'];
        filename2 = [HoloNImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename3 = [CANPath,num2str(t+(b-1)*T,'%03d'),'.mat'];
        filename4 = [PhaseNImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename5 = [AmpNImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename6 = [DepthNImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];

        save(filename1, "HoloN"); %  颗粒全息图，强度图
        imwrite(imnoise(uint8((HoloN/2 * 256)),"gaussian",0,noise_var), filename2); 
        %  强度到灰度：/2*256，单位：nan，增加高斯噪声noise_var
    ...
    % 单位：pixel, pixel, um, 绝对折射率, 相对相位,光强衰减系数
        ...
        filename1 = [HoloDPath,num2str(t+(b-1)*T,'%03d'),'.mat'];
        filename2 = [HoloDImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename3 = [CADPath,num2str(t+(b-1)*T,'%03d'),'.mat'];
        filename4 = [PhaseDImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename5 = [AmpDImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename6 = [DepthDImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];

        save(filename1, "HoloD"); %  颗粒全息图，强度图
        imwrite(imnoise(uint8((HoloD/2 * 256)),"gaussian",0,noise_var), filename2); 
        %  强度到灰度：/2*256，单位：nan，增加高斯噪声noise_var
    ...
    % 单位：pixel, pixel, um, 绝对折射率, 相对相位,光强衰减系数
        ...
        filename1 = [HoloSPath,num2str(t+(b-1)*T,'%03d'),'.mat'];
        filename2 = [HoloSImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename3 = [CASPath,num2str(t+(b-1)*T,'%03d'),'.mat'];
        filename4 = [PhaseSImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename5 = [AmpSImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        filename6 = [DepthSImgPath,num2str(t+(b-1)*T,'%03d'),'.png'];
        ...
        % 运动一帧
        ss.samples.x(N1+1:end) = ss.samples.x(N1+1:end)+Sv(:,1);
        ss.samples.y(N1+1:end) = ss.samples.y(N1+1:end)+Sv(:,2);
