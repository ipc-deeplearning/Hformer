...
ss.backGround.dz = dz; % 重新定义深度精度 单位：um/切片pixel 
...
%% 输出全息图及各种标签
    ...
    PhaseMap = zeros(FOV,FOV,length(ss.samples.x));
    AmpMap = zeros(FOV,FOV,length(ss.samples.x));
    DepthMap = zeros(FOV,FOV,length(ss.samples.x));
    ...
    PhaseMap = sum(PhaseMap,3);
    AmpMap = sum(AmpMap,3);
    DepthMap = sum(DepthMap,3);
    ...
    imwrite(uint8((Holo/2 * 256)), filename2); %  强度到灰度：/2*256，单位：nan
