...
N1 = 10; % 动态颗粒数
N2 = 10; % 静态颗粒数，取偶数
vmax = 3; % 动态颗粒的最大速度，pixel/帧
vmin = 1; % 动态颗粒的最小速度，pixel/帧
...
noise_var = 0.002; % 高斯噪声方差，仅Holo添加噪声
...
% 选择前20个随机排列的文件
numToSelect = min(numFiles, 30); % 防止选择超过文件数量的文件
...
randIndices = randperm(numFiles);
% 选择前20个随机排列的文件
numToSelect = min(numFiles, 10); % 防止选择超过文件数量的文件
selectedIndices = randIndices(1:numToSelect);
...
ss.samples.x(N1+1:end,:) = [50,60,300,300,500,100,150,450,450,350]; 
ss.samples.y(N1+1:end,:) = [50,80,200,150,370,450,480,450,350,220]; 
...
% 
%         % 获取后面 N1 行
%         lastN1Rows = ss.samples(end-N1+1:end, :);
%         
%         % 获取前面的部分，不包括后面 N1 行
%         firstPart = ss.samples(1:end-N1, :);
%         
%         % 将后面 N1 行移动到前面
%         ss.samples = [lastN1Rows; firstPart];
% 
% 
%     end
% %%
